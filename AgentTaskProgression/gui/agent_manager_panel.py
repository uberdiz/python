...
class AgentManagerPanel(ttk.Frame):
        ...
        try:
            ...
            if not loaded:
                ...
                if not self.orchestrator.load_config("default"):
                    ...
                    self.create_agent("Reviser", "reviser", "openai", "gpt-4o-mini")
                    # Added missing default agents
                    self.create_agent("ImageAgent", "image_agent", "openai", "gpt-4o-mini")
                    self.create_agent("GUICoder", "gui_coder", "openai", "gpt-4o-mini")
        ...
        self.agent_groups = [
            {
                ...
                "agents": [
                    ...
                    {"name": "Reviser", "role": "reviser", "model": "gpt-4o-mini"}
                ]
            },
            {
                "id": "anthropic_team",
                ...
                "agents": [
                    ...
                    {"name": "ClaudeReviser", "role": "reviser", "model": "claude-3-5-haiku-latest"}
                ]
            },
            {
                "id": "local_dev",
                ...
                "agents": [
                    ...
                    {"name": "LocalReviser", "role": "reviser", "model": "phi3"}
                ]
            },
            {
                "id": "mobile_app_crew",
    ...
    def save_group(self):
        """Save the current agent group to app settings with past groups dropdown"""
        if not self.orchestrator: return
        
        dialog = tk.Toplevel(self)
        dialog.title("Save Agent Group")
        dialog.geometry("350x200")
        dialog.transient(self)
        dialog.grab_set()
        
        ttk.Label(dialog, text="Group Name:").pack(pady=(20, 5), padx=20, anchor="w")
        
        # Dropdown for existing groups
        groups = self.app.settings.get("agent_groups", {})
        group_list = list(groups.keys())
        if "default" not in group_list:
            group_list.insert(0, "default")
            
        name_var = tk.StringVar(value="default")
        combo = ttk.Combobox(dialog, textvariable=name_var, values=group_list)
        combo.pack(fill="x", padx=20, pady=5)
        
        def do_save():
            name = name_var.get().strip()
            if not name:
                messagebox.showwarning("Error", "Please enter or select a group name.")
                return
            ...
            messagebox.showinfo("Saved", f"Agent group '{name}' saved successfully.")
            dialog.destroy()
            
        ttk.Button(dialog, text="Save Group", command=do_save, style="Accent.TButton").pack(pady=20)
        
        # Ensure dialog is on screen
        ensure_window_on_screen(dialog)
    ...
    def create_agent_dialog(self, edit_name=None):
        ...
        role_combo = ttk.Combobox(dialog, textvariable=role_var, values=["planner", "coder", "image_agent", "gui_coder", "tester", "reviser", "fixer"], state="readonly")
        ...
        def update_models(*args):
            ...
            if models:
                ...
                suggestions = {
                    "openai": {"planner": "gpt-4o-mini", "coder": "gpt-4o", "tester": "gpt-4o-mini", "reviser": "gpt-4o-mini"},
                    "anthropic": {"planner": "claude-3-5-sonnet-latest", "coder": "claude-3-5-sonnet-latest", "tester": "claude-3-5-haiku-latest", "reviser": "claude-3-5-haiku-latest"},
                    "google": {"planner": "gemini-1.5-pro", "coder": "gemini-1.5-pro", "tester": "gemini-1.5-flash", "reviser": "gemini-1.5-flash"},
                    "ollama": {"planner": "nemotron-3-nano", "coder": "qwen2.5-coder", "tester": "phi3", "reviser": "phi3"},
                    "huggingface": {"planner": "Qwen/Qwen2.5-7B-Instruct", "coder": "Qwen/Qwen2.5-7B-Instruct", "tester": "Qwen/Qwen2.5-7B-Instruct", "reviser": "Qwen/Qwen2.5-7B-Instruct"},
                    "deepseek": {"planner": "deepseek-chat", "coder": "deepseek-coder", "tester": "deepseek-chat", "reviser": "deepseek-chat"},
                    "mistral": {"planner": "mistral-large-latest", "coder": "mistral-large-latest", "tester": "mistral-small-latest", "reviser": "mistral-small-latest"},
                    "groq": {"planner": "llama-3.3-70b-versatile", "coder": "llama-3.3-70b-versatile", "tester": "llama-3.1-8b-instant", "reviser": "llama-3.1-8b-instant"},
                    "together": {"planner": "meta-llama/Llama-3.3-70B-Instruct-Turbo", "coder": "meta-llama/Llama-3.3-70B-Instruct-Turbo", "tester": "meta-llama/Llama-3.1-8B-Instruct-Turbo", "reviser": "meta-llama/Llama-3.1-8B-Instruct-Turbo"}
                }
                role = role_var.get()
    ...
    def open_monitor(self):
        ...
        if self.orchestrator:
            ...
            monitor = AgentMonitor(self, self.orchestrator)
            ensure_window_on_screen(monitor)
