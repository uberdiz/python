...
class PlanningPanel(ttk.Frame):
    ...
    def add_task_dialog(self):
        ...
        if desc:
            ...
            agent_name = simpledialog.askstring("Select Agent", "Enter agent role (e.g., Reviser, Planner):")
            agent = self.app.orchestrator.find_agent_by_role(agent_name)  # Use the orchestrator to find the agent
            if agent:
                self.planning_system.add_task(desc, agent)
            else:
                messagebox.showerror("Error", "Agent not found. Please enter a valid role.")
            self.refresh_all()
