...
from gui.window_utils import ensure_window_on_screen
...
class AIPanel(ttk.Frame):
    def __init__(self, parent, app):
        ...
        self.app = app
        self.model_var = tk.StringVar()  # Initialize model_var to hold selected model
    ...
    def setup_ui(self):
        ...
        # Chat container (standalone, no more monitor split)
        self.chat_container = ttk.Frame(self.chat_display_frame)
        self.chat_container.pack(fill="both", expand=True)

        # Input Area (Bottom Pane)
        self.input_area_frame = ttk.Frame(self.paned_window)
        self.paned_window.add(self.input_area_frame, weight=1) # Removed unsupported minsize
        
        # We'll use the on_resize or a timer to enforce a minimum height if needed,
        # but standard ttk.PanedWindow.add() doesn't support 'minsize' in some Tk versions.
        # Instead, we can set the height of the frame explicitly.
        self.input_area_frame.configure(height=180) 
        self.input_area_frame.pack_propagate(False) # Force the height to be respected initially
        
        # Re-enable propagation after first setup so widgets inside can be seen
        self.input_area_frame.after(100, lambda: self.input_area_frame.pack_propagate(True))
        self.setup_chat_view()
    
    def setup_chat_view(self):
        """Setup conversational chat UI"""
        # --- 1. Pending Changes Bar (Top) ---
        self.pending_changes_frame = ttk.Frame(self.chat_container, style="Action.TFrame")
        # Note: pending_changes_frame is packed dynamically when needed
        
        # --- 2. Quick Search/Action Row ---
        self.quick_row = ttk.Frame(self.chat_container)
        self.quick_row.pack(fill="x", padx=5, pady=(10, 5))
        
        # --- 3. Task List Panel (Moved to Top) ---
        self.task_container = ttk.Frame(self.chat_container)
        self.task_container.pack(fill="x", side="top", padx=5, pady=5)
        ...
        # --- 4. File Preview Area (Moved above chat history but below tasks) ---
        # Actually, let's NOT pack it here, we'll pack it in input_parent instead
        # self.preview_frame.pack(fill="x", side="top", pady=(0, 5))
        
        # Left side: Icon and Label for pending changes (content of pending_changes_frame)
        left_info = ttk.Frame(self.pending_changes_frame, style="Action.TFrame")
        left_info.pack(side="left", padx=15, pady=5)
        ...
        # --- File Preview Area (Relocated: Directly above the chat box input) ---
        self.preview_canvas = tk.Canvas(self.input_parent, height=100, highlightthickness=0, bg="#2d2d30")
        self.preview_canvas.pack(fill="x", side="top", pady=(0, 5))
        
        self.preview_scrollbar = ttk.Scrollbar(self.input_parent, orient="horizontal", command=self.preview_canvas.xview)
        ...
        def _on_preview_resize(event):
            ...
            if self.preview_frame.winfo_width() > self.preview_canvas.winfo_width():
                self.preview_scrollbar.pack(fill="x", side="top")
            else:
        ...
        self.text_input = tk.Text(text_container, height=5, bg="#202124", fg="#ffffff", 
        ...
        # Placeholder logic
        self.placeholder = "Ask anything (Ctrl+L), / for workflows"
        ...
        self.controls_container = tk.Frame(self.input_parent, bg=panel_bg)
        self.controls_container.pack(fill="x", pady=(5, 0))
        
        # Row 1: Group Selector & Auto Toggle
        row1 = tk.Frame(self.controls_container, bg=panel_bg)
        row1.pack(fill="x", pady=(0, 2))
        
        tk.Label(row1, text="Group:", bg=panel_bg, fg="#bbbbbb", font=("Segoe UI", 9)).pack(side="left", padx=(2, 0))
        ...
        self.group_combo = ttk.Combobox(row1, textvariable=self.group_var, width=15, 
        ...
        # Group Info Button
        self.info_btn = tk.Button(row1, text="ⓘ", width=2, command=self.show_group_info, 
                                 bg="#3e3e42", fg="#ffffff", relief="flat", font=("Segoe UI", 8))
        ...
        self.auto_agent_check = ttk.Checkbutton(row1, text="🚀 Auto", 
                                               variable=self.auto_agent_var,
                                               command=self.toggle_auto_agent)
        self.auto_agent_check.pack(side="right", padx=5)
        
        # Row 2: Action Buttons
        row2 = tk.Frame(self.controls_container, bg=panel_bg)
        row2.pack(fill="x", pady=2)
        
        # Consistent style for control buttons
        btn_config = {
            "bg": "#3e3e42", 
            "fg": "#ffffff", 
            "relief": "flat", 
        ...
        self.plus_btn = tk.Button(row2, text="+", width=2, command=self._show_plus_menu, **btn_config)
        self.plus_btn.pack(side="left", padx=2)
        
        # Quick Image Button
        self.img_btn = tk.Button(row2, text="📸", width=3, command=self.upload_file, **btn_config)
        self.img_btn.pack(side="left", padx=2)
        
        # Screen Capture Button
        self.cap_btn = tk.Button(row2, text="✂️", width=3, command=self.capture_screen_attachment, **btn_config)
        self.cap_btn.pack(side="left", padx=2)

        # Monitor Toggle Button
        self.monitor_btn = tk.Button(row2, text="📊 Monitor", width=8, command=self.toggle_agent_monitor_window, **btn_config)
        self.monitor_btn.pack(side="left", padx=2)
        
        # Prompt Settings Toggle
        tk.Button(row2, text="⚙️", width=3, command=self.toggle_ai_settings, **btn_config).pack(side="left", padx=2)
        
        # Enhance Button
        self.enhance_btn = tk.Button(row2, text="✨", width=3, command=self.enhance_prompt, 
                                    bg="#0e639c", fg="white", relief="flat", font=("Segoe UI", 9))
        ...
        ttk.Checkbutton(row2, text="🖼️", variable=self.auto_analyze_var, command=self._toggle_auto_analyze).pack(side="left", padx=2)

        # Right side: Mic, Stop, Send
        right_actions = tk.Frame(row2, bg=panel_bg)
        right_actions.pack(side="right")
        
        self.mic_btn = tk.Button(right_actions, text="🎤", font=("Segoe UI", 10), 
                                bg=panel_bg, fg="#d4d4d4", relief="flat",
                                command=self.toggle_voice_input)
        self.mic_btn.pack(side="left", padx=2)
        
        self.stop_btn = tk.Button(right_actions, text="■", bg="#f44336", fg="white", 
                                 width=2, relief="flat", font=("Arial", 10, "bold"),
                                 command=self.app.stop_agent)
        self.stop_btn.pack(side="left", padx=2)
        
        self.send_btn = tk.Button(right_actions, text="→", bg="#0e639c", fg="white",
                                 width=3, relief="flat", font=("Arial", 10, "bold"),
        ...
        # Update group list
        self.after(500, self.update_group_list)

    def update_group_list(self):
        """Update the list of agent groups in the dropdown"""
        if hasattr(self.app, 'settings'):
            ...
            # Set to last used or first available
            last_group = self.app.settings.get("last_agent_group", "Default")
    ...
    def on_group_selected(self, event=None):
        """Handle agent group selection and display roles/models"""
        group_name = self.group_var.get()
        if hasattr(self.app, 'agent_manager') and self.app.agent_manager.orchestrator:
            success = self.app.agent_manager.orchestrator.load_config(group_name)
            if success:
                self.app.settings["last_agent_group"] = group_name
                self.app.save_settings(self.app.settings)
                
                # List models and roles in chat
                groups = self.app.settings.get("agent_groups", {})
                group_data = groups.get(group_name, [])
                
                info_msg = f"🔄 Loaded Agent Group: **{group_name}**\n\n"
                if group_data:
                    for agent in group_data:
                        info_msg += f"• **{agent.get('role', 'Unknown')}**: {agent.get('model', 'Default')} ({agent.get('provider', 'N/A')})\n"
                else:
                    info_msg += "No agents configured for this group."
                
                self.add_chat_message("System", info_msg)
            else:
                self.add_chat_message("System", f"❌ Failed to load agent group '{group_name}'.")

    def show_group_info(self):
        """Show information about the selected agent group"""
        group_name = self.group_var.get()
        groups = self.app.settings.get("agent_groups", {})
        group_data = groups.get(group_name, [])
        ...
        if not group_data:
            ...
            messagebox.showinfo("Group Info", f"No agents defined for group '{group_name}'.")
            return

        info_text = f"Group: {group_name}\n\nAgents:\n"
        for agent in group_data:
            info_text += f"- {agent.get('name')} ({agent.get('role')}): {agent.get('model')} via {agent.get('provider')}\n"
        
        from tkinter import messagebox
        messagebox.showinfo("Agent Group Configuration", info_text)

    def toggle_agent_monitor_window(self):
        """Toggle the standalone Agent Monitor window"""
        # Check if window exists and is visible
        is_open = hasattr(self, '_monitor_window') and self._monitor_window and self._monitor_window.winfo_exists()
        
        if is_open:
            self._monitor_window.destroy()
            self._monitor_window = None
            self.monitor_btn.config(bg="#3e3e42", fg="#ffffff")
        else:
            try:
                from gui.agent_monitor import AgentMonitor
                if hasattr(self.app, 'agent_manager') and self.app.agent_manager.orchestrator:
                    self._monitor_window = AgentMonitor(self.app.root, self.app.agent_manager.orchestrator)
                    self.monitor_btn.config(bg="#0e639c", fg="white")
                    
                    # Periodically check if window was closed via X button
                    def check_monitor_status():
                        if not (self._monitor_window and self._monitor_window.winfo_exists()):
                            self.monitor_btn.config(bg="#3e3e42", fg="#ffffff")
                            self._monitor_window = None
                        else:
                            self.after(1000, check_monitor_status)
                    
                    self.after(1000, check_monitor_status)
                    
                    # Set reference in orchestrator for logging
                    self.app.agent_manager.orchestrator.agent_monitor = self._monitor_window
                else:
                    from tkinter import messagebox
                    messagebox.showwarning("No Agents", "No agent orchestrator available.")
            except Exception as e:
                self.add_chat_message("System", f"Error opening monitor: {e}")
    ...
    def add_chat_message(self, role, content):
        ...
        tag = "ai_msg" if role != "User" else "user_msg"
        if role == "System": tag = "system_msg"
    ...
    def update_file_previews(self):
        """Update the file preview area with thumbnails (Prominent display for images)"""
        # Clear existing
        ...
        if not self.attached_files:
            ...
            self.preview_scrollbar.pack_forget()
            # Restore sash if empty
            try:
                current_height = self.paned_window.winfo_height()
                if current_height > 100:
                    self.paned_window.sashpos(0, max(100, current_height - 150))
            except Exception:
                pass
            return
        ...
        header = ttk.Label(self.preview_frame, text="📸 Attached:", font=("Segoe UI", 9, "bold"))
        header.pack(side="left", anchor="center", padx=5)
        
        # Container for thumbnails
        thumb_container = ttk.Frame(self.preview_frame)
        thumb_container.pack(side="left", fill="y", padx=5)

        for i, file_path in enumerate(self.attached_files):
            frame = ttk.Frame(thumb_container, style="Card.TFrame")
            ...
            if HAS_PIL and os.path.exists(file_path):
                ...
                if ext in ['.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp']:
                    try:
                        ...
                        # Standardized thumbnail size (60x60 for scrollable)
                        pil_img.thumbnail((60, 60))
                        tk_img = ImageTk.PhotoImage(pil_img)
                        self.thumbnails[file_path] = tk_img
                        
                        lbl = ttk.Label(frame, image=tk_img, cursor="hand2")
                        lbl.pack(side="top", padx=2, pady=2)
                        lbl.bind("<Button-1>", lambda e, p=file_path: self.app.editor_tabs.open_file(p) if hasattr(self.app, 'editor_tabs') else None)
                        is_image = True
                    except Exception:
                        pass
            
            if not is_image:
                lbl = ttk.Label(frame, text="📄", font=("Segoe UI", 16))
                lbl.pack(side="top", padx=5, pady=2)
            
            # Tiny name and tiny remove button to save space
            name_lbl = ttk.Label(frame, text=name[:10], font=("Segoe UI", 7))
            name_lbl.pack(side="top")
            
            btn = ttk.Button(frame, text="×", width=2, style="Ghost.TButton",
                            command=lambda f=file_path: self.remove_attachment(f))
            btn.pack(side="top", pady=2)

        # Force paned window update to show images
        try:
            current_height = self.paned_window.winfo_height()
            if current_height > 100:
                if not self.attached_files:
                    # Restore default sash position (e.g., 150px for input)
                    self.paned_window.sashpos(0, max(100, current_height - 150))
                else:
                    # Target a reasonable area for input when images are present
                    # 100px (canvas) + 150px (input) + 70px (controls) = 320px
                    self.paned_window.sashpos(0, max(100, current_height - 320))
        except Exception:
            pass
        ...
        if file_paths:
            for path in file_paths:
                if path not in self.attached_files:
                    ...
                    icon = "📸" if path.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')) else "📎"
                    self.add_chat_message("System", f"{icon} Attached: {fname}")
            
            self.update_file_previews()
            self.text_input.focus()
            # AUTO-ANALYZE DISABLED: Wait for user to submit prompt
    ...
    def enhance_prompt(self):
        ...
        self.enhance_btn.config(state="disabled", text="⏳")
...
"""
    ...
    def _update_enhanced_prompt(self, text):
        ...
        self.enhance_btn.config(state="normal", text="✨")
    ...
    def _show_plus_menu(self):
        ...
        def safe_insert(text):
            # Ensure text input is enabled before inserting
            current_state = self.text_input.cget("state")
        ...
        menu.add_command(label="💻 Run Command", command=lambda: safe_insert("/terminal "))
        menu.add_command(label="📝 Add to Plan", command=lambda: safe_insert("/plan "))
    ...
    def send_message(self, event=None):
        ...
        target_agent = "Auto" # Default to Auto, orchestrator will decide based on group
        
        # Snapshot attached files before clearing
        current_attachments = list(self.attached_files)
        self.attached_files = []
        self.update_file_previews() # Clear the thumbnails from UI
    ...
    def _handle_request(self, request):
        ...
        # Indicate processing on main thread
        self.after(0, lambda: self.send_btn.config(state="disabled", text="⌛"))
        ...
        try:
            ...
            has_images = any(f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp')) for f in attachments)
            ...
            if message or attachments:
                ...
                first_image = attachments[0] if has_images and attachments else None
                
                if has_images:
                     display_msg = f"🖼️ [Image Analysis Request]\n{message}"
                     target_agent = "Image"
                
                self.after(0, lambda m=display_msg, img=first_image: self.add_chat_message("You", m, image_path=img))
                ...
                try:
                    if has_images and hasattr(self.app, 'run_image_analysis'):
                        # Force Image Agent first if images present
                        # Use self.app.root.after since AIDevIDE is not a widget
                        self.app.root.after(0, lambda: self.app.run_image_analysis(final_message, conversation_history=conversation_history, attachments=attachments))
                    elif hasattr(self.app, 'route_chat_to_agent'):
                        # Pass attachments explicitly since they were cleared from self.attached_files
                        self.app.route_chat_to_agent(final_message, target_agent, selected_model, conversation_history, attachments=attachments)
                    else:
            ...
            self.after(0, lambda err=e: self.add_chat_message("System", f"Processing error: {err}"))
        finally:
            # Always re-enable Send button on main thread
            self.after(0, lambda: self.send_btn.config(state="normal", text="➤"))
    ...
    def add_chat_message(self, sender, message, image_path=None):
        """Add a message to the chat history with bubble style, markdown support and optional images"""
        ...
        if image_path and os.path.exists(image_path) and HAS_PIL:
            try:
                ...
                # Display reasonably sized image in chat
                img.thumbnail((300, 300))
                tk_img = ImageTk.PhotoImage(img)
                # We need to keep a reference
                if not hasattr(self, 'chat_images'): self.chat_images = []
                self.chat_images.append(tk_img)
                
                self.chat_history.image_create("end", image=tk_img)
