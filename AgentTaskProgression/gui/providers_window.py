...
class ProvidersWindow:
    def __init__(self, parent, app):
        ...
        self.setup_ui()
        ensure_window_on_screen(self.window)

    def safe_ui_update(self, func, *args, **kwargs):
        """Safely execute UI updates only if window exists"""
    ...
    def setup_models_tab(self):
        ...
        self.model_tree = ttk.Treeview(list_frame, columns=("Model ID", "Status"), show="headings", selectmode="extended")
        ...
        ttk.Button(actions, text="Reload List", command=self.fetch_provider_models).pack(side="left", padx=5)
        ttk.Button(actions, text="Restore All", command=self.restore_all_hidden_models).pack(side="left", padx=5)
    ...
    def refresh_models_manager_list(self, event=None):
        ...
        if not self.window.winfo_exists(): return
        if not hasattr(self, 'model_tree') or not self.model_tree.winfo_exists(): return
        ...
        hidden = self.app.settings.get(f"hidden_models_{pid}", [])
        ...
        if pid == "ollama":
            ...
            for m in library:
                if m not in hidden and not any(d.startswith(m) for d in downloaded):
    ...
    def fetch_provider_models(self):
        """Fetch models for the selected provider in Model Manager (Reload)"""
        if not self.window.winfo_exists(): return
        pid = self.model_mgr_provider_var.get()
        if not pid: return
        
        self.pull_progress_var.set(f"Refreshing models for {pid}...")
        
        def do_fetch():
            # For Ollama, we don't just fetch, we want to refresh the view immediately
            # Since refresh_models_manager_list already calls fetch_available_models for Ollama
            if pid == "ollama":
                self.safe_ui_update(self.refresh_models_manager_list)
                self.safe_ui_update(self.pull_progress_var.set, "Ollama models refreshed.")
                return
            ...
            if models:
                ...
                self.safe_ui_update(self.pull_progress_var.set, f"Successfully fetched {len(models)} models.")
                self.safe_ui_update(self.refresh_models_manager_list)
            else:
                self.safe_ui_update(self.pull_progress_var.set, f"Failed to fetch models for {pid}.")
    ...
    def pull_selected_ollama_model(self):
        ...
        def run_pull():
            self.safe_ui_update(self.pull_progress_var.set, f"Pulling {model_name}...")
            ...
            def progress(s):
                ...
                self.safe_ui_update(self.pull_progress_var.set, f"Pulling {model_name}: {p}")

            success = self.app.ai_manager.pull_ollama_model(model_name, progress)
            if success:
                self.safe_ui_update(self.pull_progress_var.set, f"Successfully pulled {model_name}!")
                self.safe_ui_update(self.refresh_models_manager_list)
            else:
                self.safe_ui_update(self.pull_progress_var.set, f"Failed to pull {model_name}.")

        threading.Thread(target=run_pull, daemon=True).start()

    def pull_ollama_model_by_name(self, model_name):
        """Helper to pull an Ollama model by name (used by manual add)"""
        def run_pull():
            self.safe_ui_update(self.pull_progress_var.set, f"Pulling {model_name}...")
    ...
    def add_model_manually(self):
        ...
        if model_id:
            ...
            if model_id in hidden:
                ...
                self.app.settings[f"hidden_models_{pid}"] = hidden
                changed = True
            ...
            # If Ollama, also suggest pulling it
            if pid == "ollama":
                if messagebox.askyesno("Ollama Pull", f"Model '{model_id}' added to list. Would you like to pull/download it now?"):
                    self.pull_ollama_model_by_name(model_id)

    def delete_selected_model(self):
        """Delete selected models from cache and physically remove if local"""
        pid = self.model_mgr_provider_var.get()
        selection = self.model_tree.selection()
        ...
        for item in selection:
            m_id = self.model_tree.item(item)['values'][0]
            m_status = self.model_tree.item(item)['values'][1]
            models_to_delete.append((m_id, m_status))
            
        confirm_msg = f"Are you sure you want to remove {len(models_to_delete)} selected models?"
        # Check if any are physical deletes
        physical_deletes = [m for m in models_to_delete if m[1] == "Downloaded" and pid == "ollama"]
        if physical_deletes:
            confirm_msg = f"WARNING: {len(physical_deletes)} models will be PHYSICALLY DELETED from your storage. Continue?"
        ...
        def run_deletion():
            ...
            for m_id, m_status in models_to_delete:
                # 1. Physical deletion for local models
                if m_status == "Downloaded" and pid == "ollama":
                    self.safe_ui_update(self.pull_progress_var.set, f"Deleting {m_id}...")
                    success = self.app.ai_manager.delete_ollama_model(m_id)
                    if not success:
                        self.safe_ui_update(messagebox.showerror, "Error", f"Failed to physically delete {m_id}")
                
                # 2. Remove from IDE cache
                if m_id in cached:
                    cached.remove(m_id)
                    cache_changed = True
                
                # 3. Add to hidden list (to reduce options in UI and agents)
                if m_id not in hidden:
                    hidden.append(m_id)
                    hidden_changed = True
            ...
            if cache_changed or hidden_changed:
                self.app.save_settings(self.app.settings)
                
            self.safe_ui_update(self.pull_progress_var.set, f"Successfully processed {len(models_to_delete)} deletions.")
            self.safe_ui_update(self.refresh_models_manager_list)

        threading.Thread(target=run_deletion, daemon=True).start()

    def restore_all_hidden_models(self):
        """Restore all models that were hidden/removed from the list"""
        pid = self.model_mgr_provider_var.get()
        if not pid: return
        
        if messagebox.askyesno("Confirm", f"Restore all hidden/removed models for {pid}?"):
            if f"hidden_models_{pid}" in self.app.settings:
                del self.app.settings[f"hidden_models_{pid}"]
                self.app.save_settings(self.app.settings)
                self.refresh_models_manager_list()
                self.pull_progress_var.set(f"Restored all models for {pid}.")
    ...
    def show_provider_config(self, pid):
        ...
        def test_this_connection():
            ...
            def do_test():
                ...
                if success:
                    self.safe_ui_update(messagebox.showinfo, "Connection Test", f"Successfully connected to {pid}!\n\n{msg}")
                else:
                    self.safe_ui_update(messagebox.showerror, "Connection Test", f"Failed to connect to {pid}:\n\n{msg}")
        ...
        def refresh_models():
            ...
            def do_refresh():
                ...
                if models:
                    ...
                    self.app.save_settings(self.app.settings)
                    self.safe_ui_update(messagebox.showinfo, "Success", f"Fetched {len(models)} models for {self.active_provider_id}")
                    # Update model entry if it was empty
                    if not model_var.get() and models:
                        self.safe_ui_update(model_var.set, models[0])
                else:
                    # Restore old key if fetch failed and it was a temporary change
                    self.app.settings[f"{self.active_provider_id}_key"] = old_key
                    self.safe_ui_update(messagebox.showwarning, "Warning", f"Could not fetch models for {self.active_provider_id}. Check your API key and URL.")
