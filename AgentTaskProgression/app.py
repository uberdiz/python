...
# Apply global window fix early
try:
    from gui.window_utils import apply_global_window_fix
    apply_global_window_fix()
except Exception as e:
    print(f"Failed to apply global window fix: {e}")
...
Please provide optimized version with explanations of changes made."""
    ...
    def route_chat_to_agent(self, message, agent_name, model_name=None, conversation_history=None, attachments=None):
        """Route chat message to a specific agent"""
        if agent_name.lower() == "image":
            # Handle image analysis (Vision)
            self.run_image_analysis(message, model_name, conversation_history, attachments)
    ...
    def run_image_analysis(self, message, model_name=None, conversation_history=None, attachments=None):
        """Run the Vision/Image agent to analyze attached screenshots"""
        if not self.ai_panel:
            return
            
        # Use provided attachments or fall back to panel's current ones (though they might be cleared)
        attached = attachments if attachments else self.ai_panel.attached_files
        ...
        def worker():
            try:
                ...
                # Try to use orchestrator's image agent if available
                orchestrator = self.agent_manager.orchestrator if hasattr(self, 'agent_manager') else None
                agent = None
                
                if orchestrator:
                    # Find by role first as names can vary
                    agent_name = orchestrator.find_agent_by_role("image")
                    if agent_name and agent_name in orchestrator.agents:
                        agent = orchestrator.agents[agent_name]
                        self.log_ai(f"Using registered Image Agent '{agent_name}' ({agent.model_provider}/{agent.model_name}) from orchestrator.")
                        # If the agent is already set, we use its model and provider
                        provider = agent.model_provider
                        model = agent.model_name
                
                # Default model for vision based on provider if not already set by agent
                if not agent and not model_name:
                    if provider == "ollama":
                        # Auto-detect available vision models in Ollama
                        try:
                            import ollama
                            models_resp = ollama.list()
                            # Handle both object-based and dict-based responses from ollama library
                            models = getattr(models_resp, 'models', []) if hasattr(models_resp, 'models') else models_resp.get('models', [])
                            
                            vision_models = []
                            for m in models:
                                m_name = getattr(m, 'model', '') if hasattr(m, 'model') else m.get('name', '')
                                if 'vision' in m_name.lower():
                                    vision_models.append(m_name)
                            
                            if vision_models:
                                model = vision_models[0] # Use first available vision model
                                self.log_ai(f"Auto-detected Ollama vision model: {model}")
                            else:
                                model = self.settings.get("vision_model_ollama", "llama3.2-vision")
                        except:
                            model = self.settings.get("vision_model_ollama", "llama3.2-vision")
                    elif provider == "openai":
                        model = self.settings.get("vision_model_openai", "gpt-4o")
                    elif provider == "google":
                        model = self.settings.get("vision_model_google", "gemini-1.5-flash")
                    else:
                        model = self.settings.get("vision_model", "Qwen/Qwen2-VL-7B-Instruct")
                elif not agent:
                    model = model_name

                # Re-sync api_url and token if provider changed due to agent lookup
                api_url, _, token = self.get_ai_settings(provider)
                
                if not agent:
                    agent = ImageAgent(model_provider=provider, model_name=model, orchestrator=orchestrator)
                    agent.set_api_token(token)
                    self.log_ai(f"Created temporary Image Agent with model '{model}'.")
                else:
                    # Even for existing agent, ensure token is set
                    agent.set_api_token(token)
                
                # Final check: if provider is Ollama, ensure the model is actually available
                if provider == "ollama":
                    try:
                        import ollama
                        models_resp = ollama.list()
                        ...
                        downloaded_names = []
                        for m in models:
                            downloaded_names.append(getattr(m, 'model', '') if hasattr(m, 'model') else m.get('name', ''))
                        
                        if model not in downloaded_names:
                            # Current model not found! Try to find ANY vision model
                            vision_models = []
                            for m_name in downloaded_names:
                                if 'vision' in m_name.lower():
                                    vision_models.append(m_name)
                            
                            if vision_models:
                                new_model = vision_models[0]
                                self.log_ai(f"System: Ollama model '{model}' not found. Switching to detected vision model '{new_model}'.")
                                model = new_model
                                if agent:
                                    agent.model_name = model
                            else:
                                self.log_ai(f"Warning: No Ollama vision models found. Attempting to use '{model}' anyway.")
                    except Exception as e:
                        self.log_ai(f"Ollama check error: {e}")
                
                self.log_ai(f"🖼️ Analyzing {len(attached)} image(s)...")
                task_data = {
                ...
                # Note: ImageAgent now automatically routes to Reviser/Planner via orchestrator.
                # We only keep a fallback here if no orchestrator is present.
                if not orchestrator or not orchestrator.agents:
                    if "SUMMARY_FOR_PLANNER" in result:
                        summary_match = re.search(r'SUMMARY_FOR_PLANNER:\s*(.*?)(?:\n\n|$)', result, re.DOTALL)
                        if summary_match:
                            summary = summary_match.group(1).strip()
                            self.log_ai(f"📝 Image summary captured for planner (fallback): {summary}")
                            planning_context = f"{message}\n\n[IMAGE SUMMARY: {summary}]"
                            self.root.after(500, lambda: self.process_chat_message(planning_context, conversation_history=conversation_history))
