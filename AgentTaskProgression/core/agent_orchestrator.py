...
class AgentOrchestrator:
    ...
    def find_agent_by_role(self, role_keyword):
        """Find the first agent whose role contains the keyword (case-insensitive)"""
    ...
    def route_message(self, from_agent, to_agent, content):
        ...
        # Save to shared history for context
        message = {
            "from": from_agent,
            "to": to_agent,
            "content": content,
    ...
    def log(self, message, agent_name=None):
        """Log a message from the orchestrator or an agent"""
        ...
        if self.agent_monitor:
            try:
                # If agent_name is not provided, try to extract it from the message [name]
                target_agent = agent_name
                if not target_agent and "[" in message and "]" in message:
                    try:
                        # Extract name between brackets if present
                        target_agent = message.split("[")[1].split("]")[0]
                    except:
                        target_agent = "Orchestrator"
                
                self.agent_monitor.append_log(target_agent or "Orchestrator", message)
            except:
    ...
    def create_agent_by_role(self, name, role, provider, model):
        ...
        agent_classes = {
            ...
            "reviser": ("agents.reviser_agent", "ReviserAgent")
        }
