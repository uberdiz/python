...
class AIManager:
    ...
    def get_allowed_models(self, provider=None):
        ...
        hidden = self.app.settings.get(f"hidden_models_{provider}" if provider else "hidden_models", [])
        
        # 1. Try to get cached models first if provider specified
        if provider:
            cached = self.app.settings.get(f"cached_models_{provider}", [])
            if cached:
                return [m for m in cached if m not in hidden]
        ...
        final_list = []
        if provider in defaults:
            final_list = defaults[provider]
        else:
            # Return all defaults if no provider specified
            all_defaults = []
            for v in defaults.values(): all_defaults.extend(v)
            final_list = list(set(all_defaults))
            
        return [m for m in final_list if m not in hidden]
